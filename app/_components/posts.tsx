...
export function Posts() {
   ...
   const { data, hasNextPage, fetchNextPage, isFetching } = useInfiniteQuery({
      ...
      getPreviousPageParam: (firstPage) => firstPage.previous, // with infinite scroll it isn't necessary
   })
   ...
   return (
      <section>
         ...
         <h2>(Asynchonous Content)</h2>
         <div>
            {data?.pages.map((page, i) => (
               <ul key={i}>
                  {page.posts.map(({ full_name, id }) => (
                     <li key={id}>
                        {id}.- {full_name}
                     </li>
                  ))}
            ...
            {!hasNextPage && <div style={{ color: 'tomato', fontWeight: 600, textAlign: 'center', marginTop: '10px' }}>There is no more data</div>}
            <div style={{ textAlign: 'center', marginTop: '10px' }}>{isFetching ? <Spin /> : null}</div>
            <div ref={ref}></div>
         </div>
      </section>
   )
...
export function Spin() {
   return (
      <svg stroke="currentColor" fill="currentColor" strokeWidth="0" version="1.1" viewBox="0 0 16 16" height="1em" width="1em" className="spin" xmlns="http://www.w3.org/2000/svg">
